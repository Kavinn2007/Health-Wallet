...
-- Core metadata and master timeline entry for every patient health record
-- --------------------------------------------------------------------
...
-- Fast lookup & timeline ordering indexes
...
-- Detailed clinical notes, symptoms, and examination summaries
-- --------------------------------------------------------------------
...
-- Formal disease/condition classifications and notes
-- --------------------------------------------------------------------
...
-- Clinical interventions, therapies, and care plans
-- --------------------------------------------------------------------
...
-- Medication regimens, dosages, frequencies, and instructions
-- --------------------------------------------------------------------
...
-- Diagnostic test panels, biochemical parameters, and reference ranges
-- --------------------------------------------------------------------
...
-- --------------------------------------------------------------------
-- 8. TRIGGER: Automatic updated_at timestamp updates
-- --------------------------------------------------------------------
...
-- Strict patient profile relationship: auth.uid() -> patient_profiles.id -> records
-- No USING (true). No public FOR ALL.
-- --------------------------------------------------------------------
...
-- Helper function to resolve authenticated patient's profile ID
...
DROP POLICY IF EXISTS "Patients can update own consultations" ON public.consultations;
CREATE POLICY "Patients can update own consultations"
ON public.consultations FOR UPDATE TO authenticated
USING (patient_id = public.get_authenticated_patient_profile_id())
WITH CHECK (patient_id = public.get_authenticated_patient_profile_id());

DROP POLICY IF EXISTS "Patients can delete own consultations" ON public.consultations;
...
DROP POLICY IF EXISTS "Patients can update own prescriptions" ON public.prescriptions;
...
DROP POLICY IF EXISTS "Patients can update own lab reports" ON public.lab_reports;
CREATE POLICY "Patients can update own lab reports"
...
-- Bucket MUST be private. Never public.
-- --------------------------------------------------------------------
...
VALUES (
  ...
  'medical-records',
  false,
  10485760, -- 10 Megabytes max file size
...
-- Storage RLS: Restrict file uploads/reads to the patient's own folder
