...
-- 1. Schema Enhancements on Clinical Child Tables
-- Add care_plan to treatments if not already present
ALTER TABLE public.treatments 
ADD COLUMN IF NOT EXISTS care_plan TEXT;

-- Add start_date and end_date to prescriptions if not already present
...
-- --------------------------------------------------------------------
-- 2. SECURITY DEFINER RPC FUNCTIONS FOR DOCTOR CLINICAL CREATION
-- Identity is strictly extracted from auth.uid() — cannot be spoofed.
-- Enforces: caller is verified doctor + approved consent exists + 
--           consent not expired + consent not revoked + category permitted.
-- --------------------------------------------------------------------
